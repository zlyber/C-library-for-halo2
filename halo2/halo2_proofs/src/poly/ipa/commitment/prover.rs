...
>(
    ...
    let mut tick_MSM=start1.elapsed().as_millis();
    ...
    for j in 0..params.k {
        ...
        tick_MSM+=start3.elapsed().as_millis();
    ...
    println!("MSM time in single proof:{:?}ms",tick_MSM);
    println!("total time for single open proof:{:?} ms",start.elapsed().as_millis());
    Ok(())
}
