...
impl<E: Engine + Debug> ParamsKZG<E> {
    ...
    pub fn setup<R: RngCore>(k: u32, rng: R) -> Self {
        let n: u64 = 1 << k;
        let mut g_x: Vec<Vec<u64>> = vec![vec![0; 4]; 1<<k];
        let mut g_y: Vec<Vec<u64>> = vec![vec![0; 4]; 1<<k];
        let mut g_lagrange_x: Vec<Vec<u64>> = vec![vec![0; 4]; 1<<k];
        let mut g_lagrange_y: Vec<Vec<u64>> = vec![vec![0; 4]; 1<<k];
        let mut g2_x:Vec<u64>=vec![0;8];
        let mut g2_y:Vec<u64>=vec![0;8];
        let mut s_g2_x:Vec<u64>=vec![0;8];
        let mut s_g2_y:Vec<u64>=vec![0;8];
        let mut gx_ptr: Vec<*mut u64> = g_x.iter_mut()
            .map(|row| row.as_mut_ptr())
            .collect();
        let mut gy_ptr: Vec<*mut u64> = g_y.iter_mut()
            .map(|row| row.as_mut_ptr())
            .collect();
        let mut glagx_ptr: Vec<*mut u64> = g_lagrange_x.iter_mut()
            .map(|row| row.as_mut_ptr())
            .collect();
        let mut glagy_ptr: Vec<*mut u64> = g_lagrange_y.iter_mut()
            .map(|row| row.as_mut_ptr())
            .collect();

        unsafe{

            SETUP(k,gx_ptr.as_mut_ptr(),gy_ptr.as_mut_ptr(),glagx_ptr.as_mut_ptr(),glagy_ptr.as_mut_ptr(),g2_x.as_mut_ptr(),g2_y.as_mut_ptr(),s_g2_x.as_mut_ptr(),s_g2_y.as_mut_ptr());
            
            let mut g1: Vec<Vec<u64>> = Vec::new();
            for i in 0..g_x.len() {
            ...
            let mut g1_lagrange: Vec<Vec<u64>> = Vec::new();
            for i in 0..g_lagrange_x.len() {
                let mut tmp:Vec<u64>=Vec::new();
                tmp.extend(&g_lagrange_x[i]);
                tmp.extend(&g_lagrange_y[i]);
                g1_lagrange.push(tmp);
            }

            let mut g_affine:Vec<E::G1Affine>=Vec::new();
            let mut g_lagrange_affine:Vec<E::G1Affine>=Vec::new();
            for i in 0..g_x.len(){
                let (prefix, G, suffix) = g1[i].align_to_mut::<E::G1Affine>();
                let (prefix, G_lagrange, suffix) = g1_lagrange[i].align_to_mut::<E::G1Affine>();
                g_affine.push(G[0]);
                g_lagrange_affine.push(G_lagrange[0]);
            }
            
            let mut G_2: Vec<u64> = Vec::new();
            let mut s_G2: Vec<u64> = Vec::new();
            G_2.extend_from_slice(&g2_x);
            G_2.extend_from_slice(&g2_y);
            s_G2.extend_from_slice(&s_g2_x);
            s_G2.extend_from_slice(&s_g2_y);
            let (prefix, g_2, suffix) = G_2.align_to_mut::<E::G2Affine>();
            let (prefix, s_G_2, suffix) = s_G2.align_to_mut::<E::G2Affine>();
            let g2=g_2[0];
            let s_g2=s_G_2[0];
        ...
        // //let s = <E::Scalar>::root_of_unity();

        // let mut g_projective = vec![E::G1::group_zero(); n as usize]; 
        // parallelize(&mut g_projective, |g: &mut [<E as Engine>::G1], start| {
        //     let mut current_g: E::G1 = g1.into();   //generator=G1
        //     current_g *= s.pow_vartime(&[start as u64]); //s^i*G1
        //     for g in g.iter_mut() {
        //         *g = current_g;
        //         current_g *= s;
        //     }
        ...
        //         let offset = start + idx;//compute w^i
        //         let scalar = multiplier * root_pow * (s - root_pow).invert().unwrap(); 
        //         *g = g1 * scalar; //turn to projection coordinate
        //     }
        // });
