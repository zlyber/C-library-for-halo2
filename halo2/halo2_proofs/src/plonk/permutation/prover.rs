...
pub(crate) struct Committed<C: CurveAffine> {
    ...
    pub time_record:(u128,u128,u128),
}
...
impl Argument {
    ...
    >(
        ...
        let mut sets = vec![];
        let mut tick_MSM=0u128;
        let mut tick_IFFT=0u128;
        let mut tick_FFT=0u128;
            ...
            .zip(pkey.permutations.chunks(chunk_len))
        {
            ...
            let permutation_product_commitment_projective = params.commit_lagrange(&z, blind);//MSM
            tick_MSM+=start1.elapsed().as_millis();
            let permutation_product_blind = blind;
            let start2=Instant::now();
            let z = domain.lagrange_to_coeff(z); //IFFT
            tick_IFFT+=start2.elapsed().as_millis();
            let permutation_product_poly = z.clone();
            let start3=Instant::now();
            let permutation_product_coset = domain.coeff_to_extended(z.clone()); //FFT
            tick_FFT+=start3.elapsed().as_millis();
        ...
        let time_record=(tick_MSM,tick_IFFT,tick_FFT);
        Ok(Committed { sets,time_record })
    }
}
