...
impl<F: FieldExt> Argument<F> {
        ...
        C::Curve: Mul<F, Output = C::Curve> + MulAssign<F>,
    {
        ...
        let mut commit_values = |values: &Polynomial<C::Scalar, LagrangeCoeff>| {
            ...
            let poly = pk.vk.domain.lagrange_to_coeff(values.clone()); //IFFT difference here
            tick_IFFT+=start1.elapsed().as_micros();
            let blind = Blind(C::Scalar::random(&mut rng));
            //let blind = Blind(C::Scalar::one());
            let start2=Instant::now();    
            let commitment = params.commit_lagrange(values, blind).to_affine();  //MSM
            tick_MSM+=start2.elapsed().as_millis();
        ...
        let time_record=(tick_MSM,tick_IFFT);
        Ok(Permuted {
            compressed_input_expression,
...
impl<C: CurveAffine> Permuted<C> {
    ...
    >(
        ...
        let z = iter::once(C::Scalar::one())
            ...
            //.chain((0..blinding_factors).map(|_| C::Scalar::one()))
            .collect::<Vec<_>>();
        ...
        //let product_blind = Blind(C::Scalar::one());
        let start1=Instant::now();
        let product_commitment = params.commit_lagrange(&z, product_blind).to_affine();//MSM
        let tick_MSM=start1.elapsed().as_millis();
        let start2=Instant::now();
        let z = pk.vk.domain.lagrange_to_coeff(z);//IFFT
        let tick_IFFT=start2.elapsed().as_millis();
...
fn permute_expression_pair<'params, C: CurveAffine, P: Params<'params, C>, R: RngCore>(
    ...
    permuted_input_expression
        .extend((0..(blinding_factors + 1)).map(|_| C::Scalar::random(&mut rng))); //difference because here
    permuted_table_coeffs.extend((0..(blinding_factors + 1)).map(|_| C::Scalar::random(&mut rng)));
    
    //To eliminate the distinction, randomness is removed here.
    // permuted_input_expression
    //     .extend((0..(blinding_factors + 1)).map(|_| C::Scalar::one())); //difference because here
    // permuted_table_coeffs.extend((0..(blinding_factors + 1)).map(|_| C::Scalar::one()));
